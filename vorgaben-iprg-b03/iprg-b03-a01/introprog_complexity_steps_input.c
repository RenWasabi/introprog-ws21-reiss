...
// Initialisiert den Zufallszahlengenerator mit einem zufälligen
// Wert
...
// Befüllt die ersten len Elemente des arrays array mit Werten
// im Bereich 0 bis max_value (ausschließlich)
...
// Überprüft -- mittels memcmp -- ob die zwei übergebenen
// Arrays in den ersten len Einträgen übereinstimmen
int check_equality_of_arrays(int* array_1, int* array_2, int len)
{
    return(memcmp(array_1, array_2, len* sizeof(int)) == 0);
}

// Kopiert die ersten len Einträge -- mittels memcpy -- des
// Arrays source in das Array destination
void copy_array_elements(int* destination, int* source, int len)
{
    memcpy(destination, source, len*sizeof(int));
}

// Globale Variable, welche zum Speichern des Startzeitpunktes
// dient
long _start_timer = -1;
...
// Gibt die Zeit in Sekunden zurück, die seit dem Aufruf der
// Funktion start_timer vergangen ist
double end_timer() {
    if(_start_timer == -1) {
